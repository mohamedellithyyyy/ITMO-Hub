...
-- Sync door_count after bulk insert
-- (trigger does not exist yet during schema load)
UPDATE hall h
SET door_count = (
...
-- Fix sequences so auto-increment starts after the hardcoded IDs
SELECT setval('door_door_id_seq',
    (SELECT MAX(door_id) FROM door));
SELECT setval('hall_hall_id_seq',
    (SELECT MAX(hall_id) FROM hall));
SELECT setval('person_person_id_seq',
    (SELECT MAX(person_id) FROM person));
SELECT setval('campus_campus_id_seq',
    (SELECT MAX(campus_id) FROM campus));
SELECT setval('subject_subject_id_seq',
    (SELECT MAX(subject_id) FROM subject));
SELECT setval('mentorship_mentorship_id_seq',
    (SELECT MAX(mentorship_id) FROM mentorship));
SELECT setval('teaching_teaching_id_seq',
    (SELECT MAX(teaching_id) FROM teaching));
